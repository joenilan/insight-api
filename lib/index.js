...
/**
 * A service for Myntcore to enable HTTP routes to query information about the blockchain.
 *
...
var InsightAPI = function(options) {
  ...
  this.coinShort = options.coinShort || 'MYNT';
  this.coinTicker = options.coinTicker || 'https://api.coinmarketcap.com/v1/ticker/mynt/?convert=USD';
...
InsightAPI.dependencies = ['myntd', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  ...
  this.node.services.myntd.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.myntd.on('block', this.blockEventHandler.bind(this));
...
InsightAPI.prototype.getPublishEvents = function() {
  return [
    ...
    },
    {
        ...
        scope: this,
        subscribe: function (emitter) {
            this.subscribe(emitter, 'mynt');
        }.bind(this),
        unsubscribe: function (emitter) {
            this.unsubscribe(emitter, 'mynt');
        },
        extraEvents: ['mynt/tx', 'mynt/block']
    }
  ];
};
...
InsightAPI.prototype.blockEventHandler = function(hashBuffer) {
  ...
  if (self.subscriptions.mynt.length) {
      ...
      this.blocksController.getBlockByHash(hashBuffer.toString('hex'), function (err, block) {
          ...
          async.mapSeries(block.tx, function(txid, next) {
              ...
              // Notify mynt subscribers
              for (var i = 0; i < self.subscriptions.mynt.length; i++) {
                  self.subscriptions.mynt[i].emit('mynt/block', {
                      block: block,
...
InsightAPI.prototype.transactionEventHandler = function(txBuffer) {
   ...
   if (this.subscriptions.inv.length || this.subscriptions.mynt.length) {
       ...
       if (this.subscriptions.mynt.length) {
           var transformedTrx = this.txController.transformMyntTransaction(tx);

           for (var i = 0; i < this.subscriptions.mynt.length; i++) {
               this.subscriptions.mynt[i].emit('mynt/tx', transformedTrx);
           }
